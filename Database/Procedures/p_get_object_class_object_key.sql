DROP PROCEDURE IF EXISTS [config].[p_get_object_class_object_key];
GO

CREATE PROCEDURE [config].[p_get_object_class_object_key]
(
...
BEGIN TRY
	...
	(
		...
		,	OK.[object_key_column_id]
		,	OK.[object_key_column_name] 
		,	OK.[object_key_column_type]
		,	OK.[object_key_column_source]
		  FROM 
...
--EXEC [config].[p_get_object_class_object_key] 
